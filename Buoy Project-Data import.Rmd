...
Use lubridate to transform YYYY, MM into datetime format.
```{r}
mr_tib3 = mr_tib2 %>% 
  mutate(datetime = make_datetime(YYYY,MM))
```

```{r}
# Reorganize the tibble
mr_tib3 = mr_tib3 %>% 
  select(datetime,WD,WSPD,GST,ATMP,WTMP)
```

Use group_by to get the average of each column in monthly frequency.
```{r}
buoy = mr_tib3 %>% 
  group_by(datetime) %>% 
  summarise(mean_WD=mean(WD), mean_WSPD=mean(WSPD),
            mean_GST=mean(GST),mean_ATMP=mean(ATMP),
            mean_WTMP=mean(WTMP))
```
